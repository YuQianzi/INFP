...
from mfusion import trajectory_filter

from einops.einops import rearrange
from mfusion import MultiFeatureFusion
def to_3d(x):
...
class INFP(nn.Module):
    def get_name(self):
        return self.model_name

    def __init__(self, config, pretrained=True):
        super(INFP, self).__init__()
        ...
        self.wrn38 = wresnet(config, self.model_name, pretrained=False)  # wrn38(config, pretrained=True)
        ...
        # self.attn4 = AgentAttention(channels[3], num_patches=49)
        # self.attn2 = AgentAttention(channels[4], num_patches=49)
        ...
        self.fusion = MultiFeatureFusion(channels=2048)

        self.filter = trajectory_filter(288, size=[256,224])



        # self.wave = WaveBlock(2048)
        # self.select = RFAConv(in_channel=512,out_channel=512)
